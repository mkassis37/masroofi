...
import { formatNumber, type FinancialEntry } from "./finance-context";
...
function money(value: number, symbol: string, language: "ar" | "en", numberStyle: "arabic-indic" | "western") {
  return `${formatNumber(value, language, numberStyle)} ${symbol}`;
}
...
function reportHtml(title: string, subtitle: string, entries: FinancialEntry[], currencySymbol: string, language: "ar" | "en", numberStyle: "arabic-indic" | "western") {
  ...
  const currencySummary = Object.entries(currencyTotals).map(([code, total]) => `<div class="currency-box"><span>${localizedCurrencyName(code, findCurrency(code).name, language)} (${code})</span><strong>${money(total, findCurrency(code).symbol, language, numberStyle)}</strong></div>`).join("");
  const rows = entries.length ? entries.map((entry) => `<tr><td>${escapeHtml(formatDate(entry.date, language, numberStyle))}</td><td>${typeName(entry.type, language)}</td><td>${accountName(entry.account, language)}</td><td>${escapeHtml(entry.note)}</td><td>${money(entry.amount, currencySymbol, language, numberStyle)}</td></tr>`).join("") : `<tr><td colspan="5" class="empty">${translate("لا توجد عمليات ضمن الفترة المحددة", language, "No transactions in the selected period")}</td></tr>`;
  ...
  </style></head><body><div class="brand">${tr("مصروفي · دفتر مالي شخصي", "Masroofi · Personal finance ledger")}</div><h1>${escapeHtml(title)}</h1><div class="subtitle">${escapeHtml(subtitle)}</div><div class="summary"><div class="box income">${tr("إجمالي المدين", "Total debit")}<strong>${money(income, currencySymbol, language, numberStyle)}</strong></div><div class="box expense">${tr("إجمالي الدائن", "Total credit")}<strong>${money(expense, currencySymbol, language, numberStyle)}</strong></div><div class="box transfer">${tr("إجمالي التحويلات", "Total transfers")}<strong>${money(transfer, currencySymbol, language, numberStyle)}</strong></div></div><div class="currency-summary">${currencySummary}</div><table><thead><tr><th>${tr("التاريخ", "Date")}</th><th>${tr("النوع", "Type")}</th><th>${tr("الحساب", "Account")}</th><th>${tr("البيان", "Description")}</th><th>${tr("المبلغ", "Amount")}</th></tr></thead><tbody>${rows}</tbody></table><footer>${tr("تم إنشاء التقرير من تطبيق مصروفي", "Report generated by Masroofi")} · ${formatDate(new Date(), language, numberStyle)}</footer></body></html>`;
}

export async function generateAndSharePdf(title: string, subtitle: string, entries: FinancialEntry[], currencySymbol = "د.أ", language: "ar" | "en" = "ar", numberStyle: "arabic-indic" | "western" = language === "ar" ? "arabic-indic" : "western") {
  if (!entries.length) {
  ...
  const html = reportHtml(title, subtitle, entries, currencySymbol, language, numberStyle);
  try {
    if (Platform.OS === "web") {
