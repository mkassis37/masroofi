...
function reportHtml(title: string, subtitle: string, entries: FinancialEntry[], currencySymbol: string, language: "ar" | "en", numberStyle: "arabic-indic" | "western") {
  ...
  const rows = entries.length ? entries.map((entry) => `<tr><td>${escapeHtml(entry.date)}</td><td>${typeName(entry.type, language)}</td><td>${accountName(entry.account, language)}</td><td>${escapeHtml(entry.note)}</td><td>${money(entry.amount, currencySymbol, language, numberStyle)}</td></tr>`).join("") : `<tr><td colspan="5" class="empty">${translate("لا توجد عمليات ضمن الفترة المحددة", language, "No transactions in the selected period")}</td></tr>`;
