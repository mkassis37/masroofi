...
async function authenticate() {
  ...
  const result = await LocalAuthentication.authenticateAsync({ promptMessage: "افتح مصروفي", promptSubtitle: "تحقق من هويتك للوصول إلى بياناتك المالية", fallbackLabel: "استخدام رمز الجهاز" });
...
export function AppLock({ children }: { children: React.ReactNode }) {
  ...
  const check = async () => { if (await getLockEnabled()) setLocked(!(await authenticate())); };
  ...
  return <View style={styles.container}><Text style={styles.icon}>م</Text><Text style={styles.title}>مصروفي مقفل</Text><Text style={styles.text}>بياناتك المالية محمية. استخدم بصمة الإصبع أو رمز الجهاز للمتابعة.</Text><Pressable onPress={() => check().catch(() => undefined)} style={styles.button}><Text style={styles.buttonText}>فتح التطبيق</Text></Pressable></View>;
}
